...
public class MigrationConfigs {
    public enum MigrationProperty {
        ...
        ALLOW_RETRY("cassandra.migration.scripts.retry", "Allow to retry cql script"),
    ...
    public MigrationConfigs() {
        ...
        String allowRetryProp = System.getProperty(MigrationProperty.ALLOW_RETRY.getName());
        if(allowRetryProp != null && allowRetryProp.trim().length() != 0) {
            setRetry(allowRetryProp);
        }
    }
    ...
    public void setRetry(String retry) {
        this.retry = Boolean.parseBoolean(retry);
    }
